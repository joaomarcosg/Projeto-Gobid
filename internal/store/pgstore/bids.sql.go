...
const getBidsByProduct = `-- name: GetBidsByProduct :many
SELECT id, product_id, bidder_id, bid_amount, created_at FROM bids
WHERE product_id = $1
ORDER BY bid_amount DESC
`

func (q *Queries) GetBidsByProduct(ctx context.Context, productID uuid.UUID) ([]Bid, error) {
	rows, err := q.db.Query(ctx, getBidsByProduct, productID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
